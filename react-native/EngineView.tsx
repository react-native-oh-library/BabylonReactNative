...
import NativeEngineView, { NativeEngineViewProps, Commands as EngineViewCommands } from './NativeEngineView';
...
export const EngineView: FunctionComponent<EngineViewProps> = (props: EngineViewProps) => {
    ...
    const engineViewRef = useRef<InstanceType<typeof NativeEngineView>>(null);
            ...
            props.onInitialized({
                takeSnapshot: (): Promise<string> => {
                    if (!snapshotPromise.current) {
                        ...
                        if (engineViewRef.current) {
                            EngineViewCommands.takeSnapshot(engineViewRef.current);
                        } else {
                            throw new Error("EngineView reference not initialized");
                        }
                    }
    ...
    const snapshotDataReturnedHandler = useCallback((event:any) => {
