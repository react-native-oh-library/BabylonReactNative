...
#if __has_include("IXrContextAREngine.h")
#include "IXrContextAREngine.h"
#if __has_include("jsi/jsi.h")
namespace Babylon::Plugins::NativeXr
{
    bool TryGetXrContext(facebook::jsi::Runtime& jsiRuntime, IXrContextAREngine*& xrContext)
    {
        xrContext = nullptr;
        uintptr_t nativePtr{reinterpret_cast<uintptr_t>(nullptr)};
        if (TryGetXrContext(jsiRuntime, "AREngine", nativePtr))
        {
            xrContext = reinterpret_cast<IXrContextAREngine*>(nativePtr);
            return true;
        }
...
namespace Babylon::Plugins::NativeXr
{
    bool TryGetXrContext(Napi::Env env, IXrContextAREngine*& xrContext)
    {
        xrContext = nullptr;
        uintptr_t nativePtr{reinterpret_cast<uintptr_t>(nullptr)};
        if (TryGetXrContext(env, "AREngine", nativePtr))
        {
            xrContext = reinterpret_cast<IXrContextAREngine*>(nativePtr);
            return true;
        }
